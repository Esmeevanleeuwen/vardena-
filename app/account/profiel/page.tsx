...
export default async function ProfileSettingsPage() {
  ...
  return <SocialShell active="profile" signedIn><header className="timeline-heading"><p className="eyebrow">Jouw plek op Vardena</p><h1>Mijn profiel</h1><p>Zo zien anderen je bij berichten en in hun inbox.</p></header><section className="social-panel"><ProfileForm name={member?.display_name ?? name} username={member?.username ?? username} bio={member?.bio ?? ""}/><div className="profile-settings-links"><Link href="/organisaties">Mijn organisaties →</Link><Link href="/organisaties/nieuw">Organisatie aanmelden →</Link>{member ? <Link href={`/profiel/${member.username}`}>Bekijk je publieke profiel →</Link> : null}<Link href="/account/bevestigen">E-mailadres controleren (optioneel) →</Link></div></section></SocialShell>;
}
