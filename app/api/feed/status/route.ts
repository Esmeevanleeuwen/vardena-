...
import { feedFingerprint, feedTopic, feedType } from "@/lib/feed-types";
export async function GET(request: NextRequest) {
  const params = request.nextUrl.searchParams, supabase = await createClient();
  const result = await readFeedStatus(supabase, 1, params.get("sort") !== "nieuw", feedType(params.get("type") ?? undefined), feedTopic(params.get("onderwerp") ?? undefined));
