...
import { PersonListEditor } from "./person-list-editor";
export function PostFields({ initialKind = "post" }: { initialKind?: PostKind }) {
  ...
  return <>
    <label>Wat wil je plaatsen?<select name="kind" value={kind} onChange={e => { setKind(e.target.value as PostKind); changePhoto(null); }}><option value="post">Post met bron</option><option value="announcement">Mededeling</option><option value="photo">Foto</option><option value="list">Personenlijst</option></select></label>
    ...
    <label>{kind === "list" ? "Samenvatting" : kind === "photo" ? "Bijschrift en context" : "Je bericht"}<textarea name="body" required minLength={20} maxLength={3000} rows={3} placeholder={kind === "list" ? "Waar gaat je lijst over en waarom staan deze personen erin?" : "Wat wil je delen? Geef genoeg context…"}/></label>
    {kind === "list" ? <PersonListEditor/> : null}
    ...
    <p className="composer-guidance">{kind === "list" ? "Je lijst krijgt een eigen deelbare link. Je kunt ook per persoon een bron toevoegen." : kind === "post" ? "Een bron maakt je claim controleerbaar, niet automatisch waar." : "Een eigen mededeling of foto kan zonder bron. Doe je een feitelijke claim over iemand? Voeg dan een openbare bron toe."}</p>
  </>;
}
