...
export type PublicPost = RawPost & { member?: Member; photoUrl?: string; likes: number; dislikes: number; vote: number; reactionsAvailable: boolean };
export async function enrichPosts(supabase: SupabaseClient, posts: RawPost[], userId?: string): Promise<PublicPost[]> {
  ...
  const [stats, members, votes, photos] = await Promise.all([
  ...
  return posts.map(p => ({ ...p, photoUrl: p.media_path ? byPhoto.get(p.media_path) : undefined, member: byMember.get(p.author_id), likes: Number(byPost.get(p.id)?.likes ?? 0), dislikes: Number(byPost.get(p.id)?.dislikes ?? 0), vote: Number(byVote.get(p.id) ?? 0), reactionsAvailable: !stats.error && !votes.error }));
}

export async function readFeedStatus(supabase: SupabaseClient, page: number, popular: boolean, type: FeedType = "all", topic = "") {
  let query = supabase.from("vardena_popular_posts").select("id,updated_at,likes,dislikes",{count:"exact"});
...
export async function readTimeline(supabase: SupabaseClient, page: number, popular: boolean, type: FeedType = "all", topic = "") {
  const ranking=await readFeedStatus(supabase,page,popular,type,topic);
